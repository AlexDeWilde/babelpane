...
### Portable exe, revised: self-contained single-file, superseding the framework-dependent choice

- **Context:** The earlier "Portable exe: framework-dependent publish" decision above assumed the target machine was the user's own dev PC, which already has the .NET 10 desktop runtime installed. The user then asked for a *truly* portable exe — one file, copied to any Windows PC, running with zero prerequisites (no runtime install, no companion DLLs/config next to it) — which framework-dependent publish can't satisfy, since it still needs that runtime present.
- **Claude Code proposal / instruction received:** `dotnet publish src/BabelPane/BabelPane.csproj -c Release -r win-x64 --self-contained true -p:PublishSingleFile=true -p:IncludeNativeLibrariesForSelfExtract=true`, all four publish properties (`-r`, `--self-contained`, `PublishSingleFile`, `IncludeNativeLibrariesForSelfExtract`) passed on the command line only, nothing added to the `.csproj`, so `dotnet build`/`dotnet run` stay unaffected. `PublishTrimmed` explicitly left off — unsafe for a WPF/WinForms app this reflection-heavy on XAML without dedicated trim-compatibility work.
- **Team decision:** Accepted as specified, replacing framework-dependent as the actual "portable exe" answer for this project.
- **Reason:** Matches the stated requirement directly — zero-prerequisite, single-file portability — which framework-dependent publish structurally cannot provide.
- **Consequence:** No `.csproj` changes. Verified properly, not by trusting the publish command's exit code: copied *only* `BabelPane.exe` (172MB, bundles the full .NET runtime) out of `publish/` into an otherwise-empty folder and launched it from there — it started and stayed running with nothing else present, confirming true one-file portability. (The first publish attempt failed with `UnauthorizedAccessException` deleting the previous `publish/BabelPane.exe` — caused by a stray running process still holding that exact file open from earlier shortcut testing; killing that process and re-publishing succeeded cleanly.) Output is ~170MB versus the prior framework-dependent output's few hundred KB — an expected and accepted tradeoff for zero-prerequisite portability. The Desktop `.lnk` still points at this same `publish/BabelPane.exe` path, so it continues to work without changes, though it isn't yet the single copied-out file this decision demonstrates — copying that one file elsewhere is left for the user if/when they actually distribute it to another machine.
